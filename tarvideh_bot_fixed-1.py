...
def notify_admins(text, markup=None, photo=None):
    for aid in ADMINS:
        try:
            if photo:
                bot.send_photo(aid, photo, caption=text, reply_markup=markup)
            else:
...
def main_menu(chat_id, name=""):
    ...
    bot.send_message(chat_id,
        f"{g}"
        f"🏆 *به ربات رسمی ترویده خوش اومدی!*\n"
        f"{SEP}\n\n"
        "📌 *خدمات تخصصی ما:*\n\n"
...
def admin_panel(message):
    ...
    m.add(
        ...
        types.KeyboardButton("🧭 راهنمای پیگیری سفارش"),
...
def show_orders(chat_id, status_key=None):
    ...
    if not filtered:
        bot.send_message(chat_id, f"📭 هیچ سفارشی در وضعیت *{title}* وجود نداره.", parse_mode="Markdown"); return
    for o in filtered[-10:]:
        mk = order_action_markup(o['user_id'], o['id'])
        bot.send_message(chat_id,
            f"🔢 *سفارش #{o['id']}*\n"
            f"{SEP}\n"
            f"🛒 {o['service']}\n"
            ...
            f"🆔 `{o['user_id']}`\n"
            f"📊 وضعیت: {STATUS.get(o['status'])}\n"
            f"🧠 تشخیص: {admin_issue_summary(o['service'], o['data'])[0]}\n"
            f"📌 خلاصه: {admin_issue_summary(o['service'], o['data'])[1]}\n"
            f"🕐 {o['time']}",
...
def stats(message):
    ...
    for k, v in STATUS.items():
        text += f"{v}: {by_s.get(k, 0)}\n"
    text += f"\n📈 *سرویس‌ها:*\n"
    for s, c in sorted(by_svc.items(), key=lambda x: -x[1]):
        text += f"• {s}: {c}\n"
    bot.send_message(message.chat.id, text, parse_mode="Markdown")

@bot.message_handler(func=lambda m: m.text == "🧭 راهنمای پیگیری سفارش" and is_admin(m.from_user.id))
def admin_guide_menu(message):
    bot.send_message(message.chat.id,
        "🧭 *راهنمای کار ادمین با سفارش‌ها*\n" + SEP + "\n\n"
        "1️⃣ سفارش جدید را از بخش «در انتظار بررسی» باز کن.\n"
        "2️⃣ ابتدا تشخیص اولیه و خلاصه مشکل را بخوان.\n"
        "3️⃣ اسکرین‌شات/مدارک را بررسی کن.\n"
        "4️⃣ داخل «📂 پرونده کامل» مسیر پیگیری و متن پیشنهادی آماده را ببین.\n"
        "5️⃣ متن پیشنهادی را با اطلاعات واقعی تطبیق بده و در صورت نیاز ویرایش کن؛ ربات خودش ارسال نمی‌کند.\n"
        "6️⃣ بعد از شروع پیگیری، وضعیت را روی «در حال بررسی» یا «در حال انجام» بگذار.\n"
        "7️⃣ نتیجه را با «پاسخ به کاربر» اعلام کن و در پایان «تکمیل شد» را بزن.\n\n"
        "📌 *اصل مهم:* ربات فقط اطلاعات را جمع‌آوری و مرتب می‌کند؛ پیگیری و ارسال درخواست‌ها دستی توسط ادمین انجام می‌شود.\n"
...
@bot.message_handler(func=lambda m: is_admin(m.from_user.id) and m.text == "🔄 تازه‌سازی سفارشات")
def refresh_orders(message):
    global orders_db
    orders_db = load_orders()
    bot.send_message(message.chat.id, "✅ لیست سفارشات از دیتابیس تازه‌سازی شد.")
    show_orders(message.chat.id)
