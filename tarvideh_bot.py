...
ADMINS = [634374331]
OWNER_ID = 634374331  # مدیر اصلی — هیچکس نمیتونه حذفش کنه
CHANNEL = "@tarvideh1"
...
def ai_diagnose(user_description):
    ...
    if GROQ_API_KEY == "YOUR_GROQ_API_KEY":
        return None, "کلید API هنوز تنظیم نشده"
    try:
        headers = {
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "تو یک متخصص حرفه‌ای امنیت و بازیابی اینستاگرام هستی که برای شرکت ترویده کار می‌کنی. پاسخ‌هایت باید دقیق، کاربردی و فقط به فارسی باشد."
                },
                {
                    "role": "user",
                    "content": f"""کاربر این مشکل اینستاگرامی رو داره:
{user_description}

لطفاً به فارسی و حرفه‌ای تحلیل کن:

1️⃣ نوع مشکل: دقیقاً چه نوع مشکلیه؟
2️⃣ احتمال حل: چند درصد قابل حله و چرا؟
3️⃣ دلیل احتمالی: چرا این اتفاق افتاده؟
4️⃣ اقدام فوری: ۳ کار که باید همین الان بکنه؟
5️⃣ خدمت ترویده: کدام سرویس مناسبه؟ (بازگردانی، رفع محدودیت، امنیت پیج، حذف پیج جعلی)
6️⃣ هشدار: چه کاری نباید بکنه؟

پاسخ رو کوتاه، واضح و با ایموجی بده."""
                }
            ]
        }
        r = requests.post(
        ...
        if r.status_code == 200:
            ...
            print(f"Groq error {r.status_code}: {err}")
            return None, err
