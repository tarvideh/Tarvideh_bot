...
def smart_parse_page_info(raw_text):
    """
    اطلاعات پیج رو چه به فرمت 'کلید: مقدار' چه به‌صورت متن درهم و پراکنده بفرستی،
    این تابع خودش تشخیص میده و مرتبش می‌کنه.
    """
    info = {}
    remaining = raw_text

    # مرحله ۱: خط‌هایی که دقیقاً به فرمت "کلید: مقدار" هستن رو جدا کن (اولویت با این‌هاست)
    kept_lines = []
    ...
    # مرحله ۲: از باقیمانده متن (که ممکنه کاملاً درهم باشه)، با الگو استخراج کن
    # ایمیل
    if "ایمیل" not in info and "email" not in info:
        m = re.search(r'[A-Za-z0-9_.+-]+@[A-Za-z0-9-]+\.[A-Za-z0-9.-]+', remaining)
        if m:
            info["ایمیل"] = m.group(0)
            remaining = remaining.replace(m.group(0), " ")

    # آیدی اینستاگرام (@username) — چیزی که با @ شروع میشه ولی ایمیل نیست
    if "آیدی" not in info and "username" not in info:
        for m in re.finditer(r'@[A-Za-z0-9_.]{2,30}', remaining):
            info["آیدی"] = m.group(0)
            remaining = remaining.replace(m.group(0), " ", 1)
            break

    # شماره تلفن ایرانی
    if "تلفن" not in info and "phone" not in info:
        m = re.search(r'(?<!\d)(?:\+98|0098|0)?9\d{9}(?!\d)', remaining)
        if m:
            info["تلفن"] = m.group(0)
            remaining = remaining.replace(m.group(0), " ", 1)

    # تعداد فالوور — عددی که کنارش کلمه فالوور/follower یا پسوند k/هزار باشه
    if "فالوور" not in info and "followers" not in info:
        m = re.search(r'(\d[\d,.]*\s*[kKKمهزار]*)\s*(?:فالوور|follower|فالووِر)', remaining, re.IGNORECASE)
        if not m:
            m = re.search(r'(?:فالوور|follower)\D{0,5}(\d[\d,.]*\s*[kK]?)', remaining, re.IGNORECASE)
        if not m:
            m = re.search(r'\b(\d[\d,.]*\s*[kK])\b', remaining)
        if m:
            info["فالوور"] = m.group(1).strip()
            remaining = remaining.replace(m.group(0), " ", 1)

    # نوع پیج
    if "نوع پیج" not in info:
        if re.search(r'\bکاری\b', remaining):
            info["نوع پیج"] = "کاری"
        elif re.search(r'\bشخصی\b', remaining):
            info["نوع پیج"] = "شخصی"

    # هرچی از متن باقی موند = توضیحات کامل مشکل
    leftover = re.sub(r'\s+', ' ', remaining).strip(" \n\t-،,:")
    if leftover:
        if "توضیحات" in info:
            info["توضیحات"] = info["توضیحات"] + "\n" + leftover
        else:
            info["توضیحات"] = leftover

    return info


def build_fill_data(info):
    return {
...
# نگاشت فیلدهای ذخیره‌شده در سفارش مشتری به فیلدهای موردنیاز قالب ایمیل
ORDER_DATA_TO_INFO_MAP = {
    "ig_id": "آیدی", "email": "ایمیل", "phone": "تلفن", "followers": "فالوور",
    "desc": "توضیحات", "page_type": "نوع پیج", "reason": "دلیل", "target_id": "پیج هدف",
}

def safe_send_markdown(chat_id, text, reply_markup=None):
    """
    ارسال پیام با Markdown؛ اگه متن (مثلاً به‌خاطر کاراکترهای خاص _ * ` توی توضیحاتی
    که ادمین/کاربر آزادانه تایپ کرده) توسط تلگرام رد بشه، به‌جای گم‌شدن بی‌صدای پیام،
    همون متن رو بدون فرمت (plain text) دوباره می‌فرسته تا چیزی از دست نره.
    """
    try:
        return bot.send_message(chat_id, text, parse_mode="Markdown", reply_markup=reply_markup)
    except Exception as e:
        print(f"safe_send_markdown: Markdown failed ({e}); retrying as plain text")
        try:
            return bot.send_message(chat_id, text, reply_markup=reply_markup)
        except Exception as e2:
            print(f"safe_send_markdown: plain text also failed: {e2}")
            try:
                bot.send_message(chat_id, "⚠️ خطا در نمایش پیش‌نمایش ایمیل. لطفاً دوباره امتحان کن.")
            except Exception:
                pass
            return None

def safe_edit_markdown(chat_id, message_id, text, reply_markup=None):
    """نسخه ایمن edit_message_text — اگه Markdown ادیت رو خراب کنه، بدون فرمت ادیت می‌کنه،
    و اگه ادیت اصلاً ممکن نبود (مثلاً پیام قدیمیه)، یه پیام جدید می‌فرسته."""
    try:
        return bot.edit_message_text(text, chat_id, message_id, parse_mode="Markdown", reply_markup=reply_markup)
    except Exception as e:
        print(f"safe_edit_markdown: Markdown edit failed ({e}); retrying plain edit")
        try:
            return bot.edit_message_text(text, chat_id, message_id, reply_markup=reply_markup)
        except Exception as e2:
            print(f"safe_edit_markdown: plain edit also failed ({e2}); sending new message instead")
            return safe_send_markdown(chat_id, text, reply_markup)

WIZARD_STEPS = [
    ("wiz_username", "آیدی",   "1️⃣ آیدی پیج اینستاگرام رو بفرست:\n_(مثال: @username)_"),
    ("wiz_name",     "نام",    "2️⃣ اسم صفحه رو بفرست:"),
    ("wiz_email",    "ایمیل",  "3️⃣ ایمیل متصل به پیج رو بفرست:"),
    ("wiz_phone",    "تلفن",   "4️⃣ شماره تلفن رو بفرست:"),
    ("wiz_followers","فالوور", "5️⃣ تعداد فالوور رو بفرست:"),
    ("wiz_pagetype", "نوع پیج","6️⃣ نوع پیج چیه؟ (کاری یا شخصی)"),
    ("wiz_desc",     "توضیحات","7️⃣ توضیح کامل مشکل رو بفرست:"),
]

def wizard_prompt(chat_id, uid, step_idx):
    step_key, label, question = WIZARD_STEPS[step_idx]
    user_data[f"email_step_{uid}"] = step_key
    mk = types.InlineKeyboardMarkup()
    mk.add(types.InlineKeyboardButton("⏭ ندارم / رد کن", callback_data=f"wiz_skip_{uid}"))
    bot.send_message(chat_id,
        f"📝 *ورود قدم‌به‌قدم اطلاعات پیج* ({step_idx+1}/{len(WIZARD_STEPS)})\n{SEP}\n\n{question}",
        parse_mode="Markdown", reply_markup=mk)

def wizard_finish(chat_id, uid):
    info = user_data.pop(f"wiz_data_{uid}", {})
    ...
    fill_data = build_fill_data(info)
    text, mk = show_email_template(chat_id, uid, em_type, to_email, fill_data, sender, 0)
    safe_send_markdown(chat_id, text, reply_markup=mk)

@bot.callback_query_handler(func=lambda c: c.data.startswith("wiz_skip_"))
def wizard_skip(call):
    uid = call.from_user.id
    if not is_admin(uid): return
    cur_step = user_data.get(f"email_step_{uid}", "")
    idx = next((i for i, s in enumerate(WIZARD_STEPS) if s[0] == cur_step), None)
    bot.answer_callback_query(call.id, "رد شد")
    if idx is None:
        return
    if idx + 1 < len(WIZARD_STEPS):
        wizard_prompt(call.message.chat.id, uid, idx + 1)
    else:
        wizard_finish(call.message.chat.id, uid)

@bot.message_handler(func=lambda m: user_data.get(f"email_step_{m.from_user.id}", "") in [s[0] for s in WIZARD_STEPS])
def wizard_answer(message):
    uid = message.from_user.id
    if message.text == "🔙 بازگشت به منو":
        for k in [f"email_to_{uid}", f"email_type_{uid}", f"email_step_{uid}", f"wiz_data_{uid}"]:
            user_data.pop(k, None)
        go_back(message); return
    ...
    if idx is None:
        return
    _, label, _ = WIZARD_STEPS[idx]
    wiz_data = user_data.setdefault(f"wiz_data_{uid}", {})
    wiz_data[label] = message.text.strip()

    if idx + 1 < len(WIZARD_STEPS):
        wizard_prompt(message.chat.id, uid, idx + 1)
    else:
        wizard_finish(message.chat.id, uid)

@bot.callback_query_handler(func=lambda c: c.data.startswith("ordlook_"))
def order_lookup_start(call):
    uid = call.from_user.id
    if not is_admin(uid): return
    user_data[f"email_step_{uid}"] = "order_lookup"
    bot.send_message(call.message.chat.id,
        "🗂 شماره سفارش مشتری رو بفرست (فقط عدد):\n"
        "_(مثال: 128 — همون شماره‌ای که کنار «سفارش #» توی اعلان‌ها می‌بینی)_",
        parse_mode="Markdown", reply_markup=back_btn())
    bot.answer_callback_query(call.id)

@bot.message_handler(func=lambda m: user_data.get(f"email_step_{m.from_user.id}", "") == "order_lookup")
def order_lookup_receive(message):
    uid = message.from_user.id
    if message.text == "🔙 بازگشت به منو":
        for k in [f"email_to_{uid}", f"email_type_{uid}", f"email_step_{uid}"]:
            user_data.pop(k, None)
    ...
    if not raw.isdigit():
        bot.send_message(message.chat.id, "❌ فقط شماره سفارش رو به‌صورت عدد بفرست.")
        return

    oid = int(raw)
    order = next((o for o in orders_db if o["id"] == oid), None)
    if not order:
        bot.send_message(message.chat.id, f"❌ سفارشی با شماره #{oid} پیدا نشد. دوباره بفرست:")
        return

    d = order.get("data", {})
    info = {"نام": order.get("user_name", "---")}
    for data_key, info_key in ORDER_DATA_TO_INFO_MAP.items():
        if d.get(data_key):
            info[info_key] = d[data_key]
    fill_data = build_fill_data(info)
    ...
    safe_send_markdown(message.chat.id, f"✅ اطلاعات از سفارش #{oid} گرفته شد:\n{recap}")

    text, mk = show_email_template(message.chat.id, uid, em_type, to_email, fill_data, sender, 0)
    safe_send_markdown(message.chat.id, text, reply_markup=mk)
...
def email_type_selected(call):
    ...
    mk = types.InlineKeyboardMarkup(row_width=1)
    mk.add(
        types.InlineKeyboardButton("📝 قدم‌به‌قدم بپرس ازم", callback_data=f"wizstart_{call.from_user.id}"),
        types.InlineKeyboardButton("🗂 از سفارش مشتری بگیر", callback_data=f"ordlook_{call.from_user.id}"),
    )
    bot.send_message(call.message.chat.id,
        "📝 اطلاعات پیج رو بفرست — به هر شکلی، حتی درهم و بدون فرمت خاص، خودم مرتبش می‌کنم:\n\n"
        "آیدی: @username\n"
        "نام: اسم پیج\n"
        ...
        "👇 یا اگه ترجیح میدی، یکی از این دو گزینه رو انتخاب کن:",
        reply_markup=mk)
    bot.send_message(call.message.chat.id, "بازگشت:", reply_markup=back_btn())
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("wizstart_"))
def wizard_start(call):
    uid = call.from_user.id
    if not is_admin(uid): return
    user_data[f"wiz_data_{uid}"] = {}
    wizard_prompt(call.message.chat.id, uid, 0)
    bot.answer_callback_query(call.id)
...
def email_info_received(message):
    ...
    info = smart_parse_page_info(message.text.strip())
    fill_data = build_fill_data(info)
...
def template_nav(call):
    ...
    if call.data == f"tpl_copy_{uid}":
        ...
        safe_send_markdown(call.message.chat.id,
    ...
    safe_edit_markdown(call.message.chat.id, call.message.message_id, text, reply_markup=mk)
    bot.answer_callback_query(call.id)
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "تو یک کارشناس پشتیبانی اینستاگرام هستی که برای تیم ترویده کار می‌کنی. "
                    "مهم‌ترین وظیفه‌ات اینه که دقیقاً به همون سوال یا مشکلی که کاربر مطرح کرده جواب بدی، "
                    "نه یک قالب ثابت و یکسان برای همه‌ی سوال‌ها. هر پاسخ باید کاملاً مختص همون مورد باشه.\n"
                    "قوانین نوشتن پاسخ:\n"
                    "- فقط فارسی، ساده و روان بنویس؛ طوری که یک بچه ۱۰ ساله هم کامل بفهمدش.\n"
                    "- از اصطلاحات فنی یا پیچیده استفاده نکن؛ اگر مجبور شدی، همون‌جا با چند کلمه ساده توضیحش بده.\n"
                    "- راه‌حل رو همیشه به‌صورت مرحله‌ای و شماره‌گذاری‌شده (۱، ۲، ۳...) بنویس، دقیقاً به همون تعدادی که لازمه (نه کم نه زیاد و نه الکی طولانی).\n"
                    "- هیچ بخشی که به سوال کاربر مربوط نیست رو ننویس. اگه سوال ساده است، جواب هم باید کوتاه و مستقیم باشه.\n"
                    "- بدون جدول، بدون HTML، بدون مارک‌داون؛ فقط متن ساده و ایموجی مناسب.\n"
                    "- هرگز جواب کلی، حدسی یا نامرتبط نده."
                )
            },
            {
                "role": "user",
                "content": f"""سوال یا مشکل دقیق کاربر این است:
«{user_description}»

اول (فقط برای خودت، در پاسخ ننویس) مشخص کن کاربر دقیقاً چی پرسیده یا چه مشکلی داره.
بعد پاسخ رو با همین ساختار بنویس، ولی هر بخشی که واقعاً به همین سوال مربوط نیست رو کامل حذف کن:

🔍 مشکل شما:
[مشکل دقیق کاربر رو با زبان خیلی ساده و با کلمات خودش بازگو کن، تا مطمئن بشه درست فهمیدیش]

✅ راه‌حل گام‌به‌گام:
۱. [قدم اول، خیلی دقیق و ساده، مثل توضیح دادن به یک بچه ۱۰ ساله]
۲. [قدم دوم]
۳. [قدم سوم، اگر لازم بود]
(فقط تعداد قدم‌هایی که واقعاً لازمه بنویس)

⚠️ نکته مهم:
[فقط اگر یک هشدار یا نکته امنیتی واقعاً مرتبط با همین مشکل وجود داره؛ در غیر این صورت این بخش رو کلاً حذف کن]

🛡 کمک تخصصی ترویده:
[فقط زمانی این بخش رو بنویس که مشکل کاربر واقعاً نیاز به کمک تخصصی و پولی تیم ترویده داره، مثل دیسیبل شدن پیج، هک شدن، یا محدودیت شدید. اگر مشکل ساده‌ای است که کاربر خودش با همون راهنمای بالا حلش می‌کنه (مثلاً فراموشی رمز عبور ساده)، این بخش رو کلاً ننویس]

مهم: از تکرار یک قالب یکسان برای همه‌ی سوالات خودداری کن. پاسخ باید کاملاً برای همین یک سوال نوشته شده باشه، نه یک متن عمومی.
مهم: خط پایانی «برای کمک به پشتیبانی ترویده مراجعه کن» رو خودت ننویس، این خط به‌صورت خودکار به پاسخ اضافه می‌شه."""
            }
        ]
        ...
        for model in GROQ_MODELS:
            try:
                ...
                if r.status_code == 200:
                    ...
                    closing_note = (
                        f"\n\n{SEP}\n"
                        "💡 اگه توی حل این مشکل به کمک نیاز داشتی، از پشتیبانی ترویده کمک بگیر — "
                        "سریع‌تر و مطمئن‌تر برات انجامش میدن:\n"
                        "👤 @tarvideh"
                    )
                    return ai_text + closing_note, None
                else:
